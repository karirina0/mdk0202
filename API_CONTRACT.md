...
Версия: 1.0 (согласована совместно всей командой на Этапе 2, без изменений
дошла до финального релиза — конфликт при слиянии возник в другом файле,
см. `REPORT.md`)
...
## 5. Как контракт согласовывался в команде

Документ спроектирован совместно всеми тремя участниками на Этапе 2, до
создания feature-веток, и загружен Tech Lead'ом в ветку `dev` через
веб-интерфейс GitHub (**Add file → Upload files**). Разработчики Task
Service и Notification Service во время параллельной разработки (Этап 3)
этот файл не редактировали — каждый работал только в папке своего сервиса
(`task-service/` и `notification-service/`), поэтому при слиянии
`feature/tasks-service → dev` и `feature/notifications-service → dev`
конфликтов в `API_CONTRACT.md` не возникло. Единственный конфликт при
интеграции произошёл в `README.md` на финальном PR `dev → main` — подробно
разобран в `REPORT.md`.
